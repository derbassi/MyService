...
.EXAMPLE
Get-MyService1 -ServiceName net -status Running | Format-Table -Autosize
...
.EXAMPLE
Get-MyService1 -ServiceName "xbox" -status Stopped | Format-Table -Autosize
...
NlaSvc   Network Location Awareness      2584 Auto         True Running
.EXAMPLE
Get-MyService1 -ServiceName L -Status AllStatus | ? {$_.startMode -eq 'Disabled'} | Format-Table -AutoSize

Name       DisplayName            ProcesSId StartMode Started State  
----       -----------            --------- --------- ------- -----  
AppVClient Microsoft App-V Client         0 Disabled    False Stopped
.EXAMPLE
Get-MyService1 -DisplayName search -Status Running | ? {$_.startMode -eq "Auto"}
...
#region functions
# s = service, d=disabled, r=result, Example $rd = result of the disabled
...
Function Get-MyService{
  ...
  $r = $s |  Where-Object {$_.State -eq $Status}     
   ...
   if($status -in ("Running","Stopped")){
     ...
     if($Null -eq $r){
      return Write-Host "Are You sure about the service?" -ForegroundColor Yellow -BackgroundColor Red
 ...
 Function Get-StartModeMyService{
  ...
  $rd = Get-WmiObject -Class Win32_Service | Select-Object Name, DisplayName, Started, State, StartMode
   ...
   switch($StartMode){
     ...
     'Disabled'{
     $rd | Where-Object {$_.StartMode -eq "Disabled"} 
     break; 
     }
   }
 }
 ...
 Function Update-MyServiceStartMode{
 [CmdletBinding()]
